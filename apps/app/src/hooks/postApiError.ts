  ...
  if (field) {
    const fieldIssue = issues.find(
      (issue) =>
        ...
        issue.path.some((part) => String(part) === field) &&
...
export async function readApiErrorMessage(
  ...
  try {
    const jsonSource = response.clone ? response.clone() : response;
    payload = (await jsonSource.json()) as ApiErrorPayload;
  } catch {
    payload = null;
  }

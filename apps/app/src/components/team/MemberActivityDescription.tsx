...
import {
  ACTIVITY_ACTIONS,
  type ActivityAction,
} from "@featul/api/shared/activity-actions"
...
const ACTIVITY_COPY: Partial<Record<ActivityAction, ActivityCopyConfig>> = {
  ...
  [ACTIVITY_ACTIONS.COMMENT_VOTE_REMOVED]: {
    label: "removed vote from a comment on",
  },
  [ACTIVITY_ACTIONS.COMMENT_VOTE_CHANGED]: {
    label: "changed vote on a comment on",
  },
  [ACTIVITY_ACTIONS.COMMENT_MARKED_INTERNAL]: {
    label: "marked comment as internal on",
  },
  [ACTIVITY_ACTIONS.COMMENT_MARKED_EXTERNAL]: {
    label: "marked comment as external on",
  },
...
function renderStatusWithLabel(status: unknown) {
  ...
  return (
    <span className="inline-flex items-center gap-1 shrink-0">
      <StatusIcon status={String(status)} className="size-3 shrink-0" />
      <span className="text-[10px] text-accent">{label}</span>
    </span>
  )
}

function renderTitledActivityRow(
  item: ActivityItem,
  status: unknown,
  config: ActivityCopyConfig,
) {
  ...
  return (
    <span className="flex items-center gap-2 min-w-0">
      ...
      {includeTitle && item.title ? (
        <span className={TITLE_CLASS} title={item.title}>
          {item.title}
        </span>
      ) : null}
    </span>
...
function renderInlineTagSummary(tags: TagSummary[]) {
  ...
  return (
    <span className="flex items-center gap-1.5 shrink-0 text-xs text-accent">
      {visibleTags.map((tag) => (
        <span
...
export function MemberActivityDescription({ item }: { item: ActivityItem }) {
  ...
  const tags: TagSummary[] =
    ...
    (Array.isArray(item.metadata?.tagSummaries) &&
      item.metadata?.tagSummaries) ||
    []
    ...
    if (item.type === ACTIVITY_ACTIONS.TAG_CREATED) {
      return (
        <span className="flex items-center gap-2 min-w-0">
          ...
          <span className="inline-flex items-center gap-2 rounded-full border border-border/70 bg-muted/80 ring-1 ring-border/60 ring-offset-1 ring-offset-background px-2 py-0.5 text-xs">
            {color ? (
              <span
    ...
    if (item.type === ACTIVITY_ACTIONS.TAG_DELETED) {
      return (
        <span className="flex items-center gap-2 min-w-0">
          ...
          <span className="inline-flex items-center gap-2 rounded-full border border-border/70 bg-muted/60 ring-1 ring-border/60 ring-offset-1 ring-offset-background px-2 py-0.5 text-xs">
            <span className="truncate max-w-[160px]">{label}</span>
          </span>
        </span>
      )
    ...
    if (item.type === ACTIVITY_ACTIONS.CHANGELOG_TAG_CREATED) {
      return (
        <span className="flex items-center gap-2 min-w-0">
          ...
          <span className="inline-flex items-center gap-2 rounded-full border border-border/70 ring-1 ring-border/60 ring-offset-1 ring-offset-background bg-muted/80 px-2 py-0.5 text-xs">
            {color ? (
              <span
                className="inline-block size-2 rounded-full"
                style={{ backgroundColor: color }}
    ...
    if (item.type === ACTIVITY_ACTIONS.CHANGELOG_TAG_DELETED) {
      return (
        <span className="flex items-center gap-2 min-w-0">
          ...
          {label ? (
            <span className="inline-flex items-center rounded-full border border-border/70 bg-muted/60 px-2 py-0.5 text-xs">
              <span className="truncate max-w-[160px]">{label}</span>
            </span>
          ) : null}
        </span>

...
export default function RequestDetail({
  ...
  const author = getDisplayUser(post.author ? { name: post.author.name ?? undefined, image: post.author.image ?? undefined, email: post.author.email ?? undefined } : undefined)
    ...
    >
      ...
      <article className="min-w-0">
        <header className="space-y-4">
          <h1 className="text-xl font-semibold leading-snug tracking-tight wrap-break-word text-foreground sm:text-2xl">{post.title}</h1>
          <div className="flex items-center gap-2.5">
            ...
            <div className="min-w-0">
              ...
              <time dateTime={post.publishedAt ?? post.createdAt} className="text-xs text-accent">{relativeTime(post.publishedAt ?? post.createdAt)}</time>
            </div>
          </div>
        </header>

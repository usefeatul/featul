        ...
        >
          {prevHref ? (
            <Link href={prevHref} title={prev?.title ? `Previous: ${prev.title} (Z)` : "Previous (Z)"} aria-label="Previous post" aria-keyshortcuts="z">
              ...
              <span className="hidden sm:inline-flex items-center justify-center rounded-sm border bg-[var(--workspace-surface)] px-1.5 text-xs font-extralight text-accent tabular-nums h-5">Z</span>
            </Link>
          ) : (
            <span aria-hidden="true" className="flex items-center gap-2">
        ...
        >
          {nextHref ? (
            <Link href={nextHref} title={next?.title ? `Next: ${next.title} (X)` : "Next (X)"} aria-label="Next post" aria-keyshortcuts="x">
              <span className="hidden sm:inline-flex items-center justify-center rounded-sm border bg-[var(--workspace-surface)] px-1.5 text-xs font-extralight text-accent h-5">X</span>

...
export function RequestOverflowMenu({
  ...
  const toggleFlag = async (key: RequestFlagKey) => {
    if (!flags || !onFlagsChange || savingFlag) return
    setSavingFlag(true)
    try {
      const patch: RequestFlags = { [key]: !flags[key] }
      await client.board.updatePostMeta.$post({ postId, ...patch })
      onFlagsChange({ ...flags, ...patch })
    } finally {
      setSavingFlag(false)
    }
  }
      ...
      <Popover open={menuOpen} onOpenChange={setMenuOpen}>
        <PopoverTrigger asChild>
          <Button
            ...
            variant="nav"
            size="icon-sm"
            className="rounded-none border-none shadow-none hover:bg-background focus-visible:ring-0 focus-visible:ring-offset-0"
                ...
                {REQUEST_FLAG_OPTIONS.map((option) => {
                  ...
                  return (
                    <PopoverListItem
                      ...
                      aria-disabled={savingFlag}
                    >
                      <span className="text-sm">{option.label}</span>
                      {checked ? <span className="ml-auto text-xs">✓</span> : null}
                    </PopoverListItem>
                  )
                })}
              </>

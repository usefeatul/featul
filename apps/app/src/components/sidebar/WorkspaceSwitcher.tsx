      ...
      <DropdownMenu open={open} onOpenChange={setOpen}>
        <DropdownMenuTrigger className="w-full cursor-pointer rounded-md">
          <div className="group flex items-center gap-2 rounded-md px-3 py-2 cursor-pointer transition-colors hover:bg-muted">
            <div className={cn("relative size-7 overflow-hidden rounded-md bg-card", currentLogo ? "bg-transparent" : "bg-card")}>
              {currentLogo ? (
                <Image
                  src={currentLogo}
                  alt={currentName}
            ...
            <span className="ml-auto inline-flex h-5 w-5 shrink-0 items-center justify-center rounded-sm bg-card text-xs font-medium text-accent">
                ...
                <div className="flex flex-col gap-1 pb-1">
                  {all.map((w) => {
                    ...
                    return (
                      ...
                      >
                        {logoUrl ? (
                          <div className="relative w-8 h-8 shrink-0 overflow-hidden rounded-md bg-muted">
                            <Image
                              src={logoUrl}
                              alt={w.name}
                              fill

...
export default function AvatarUpload({ initialUser }: AvatarUploadProps) {
  ...
  const onAvatarFile = React.useCallback(
    async (file: File) => {
      ...
      try {
        ...
        const { error, data: updated } = await authClient.updateUser({
          image: publicUrl,
        });
        if (error) {
  ...
  const onAvatarDrop: React.DragEventHandler<HTMLButtonElement> =
    React.useCallback(
      (e) => {
        e.preventDefault();
        if (uploadingImage) return;
  ...
  const onAvatarDragOver: React.DragEventHandler<HTMLButtonElement> =
    React.useCallback((e) => {
      e.preventDefault();
    }, []);
        ...
        >
          <span className="flex items-center gap-2">
            <Avatar className="size-5">
              {image.trim() || d.image ? (
                <AvatarImage src={image.trim() || d.image || ""} alt={d.name} />
              ) : null}

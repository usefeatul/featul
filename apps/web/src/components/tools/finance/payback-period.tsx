...
import { parseNumericInput } from "@/components/tools/global/numbers";
import {
  Card,
  CardHeader,
  CardTitle,
...
export default function PaybackPeriodTool() {
  ...
  const months = React.useMemo(() => {
    const c = parseNumericInput(cac);
    const a = parseNumericInput(arpu);
    const gm = parseNumericInput(grossMarginPct) / 100;
    const denom = a * gm;
    if (denom <= 0) return 0;
    return c / denom;
  ...
  return (
    <div>
      <div className="prose prose-sm sm:prose-base prose-zinc dark:prose-invert mt-6 tracking-wide">
        ...
        <p>
          Months to recover CAC given ARPU and gross margin. Formula:{" "}
          <code>Payback = CAC ÷ (ARPU × Gross margin)</code>. Use this
          calculator to estimate how many months it takes to recover customer
          acquisition cost. Enter CAC, monthly ARPU, and gross margin
          percentage.
        </p>
      </div>
      ...
      <div className="mt-6 grid grid-cols-1 gap-4">
            ...
            <div className="grid grid-cols-2 md:grid-cols-4 gap-3 items-end">
              ...
              <div className="space-y-1">
                ...
                <Input
                  id="pb"
                  type="text"
                  value={`${formatNumber(months)} months`}
                  readOnly
                />
              </div>
            </div>
            ...
            <div className="grid grid-cols-2 md:grid-cols-4 gap-4 items-stretch">
              ...
              <div className="rounded-md  border p-3 text-center flex flex-col items-center justify-center min-h-[72px]">
                ...
                <div className="mt-1 font-mono text-base leading-tight text-foreground">
                  {parseNumericInput(cac).toLocaleString()}
                </div>
              </div>
              <div className="rounded-md  border p-3 text-center flex flex-col items-center justify-center min-h-[72px]">
                <div className="text-xs text-accent">ARPU</div>
                <div className="mt-1 font-mono text-base leading-tight text-foreground">
                  {parseNumericInput(arpu).toLocaleString()}
                </div>
              </div>
              <div className="rounded-md  border p-3 text-center flex flex-col items-center justify-center min-h-[72px]">
                <div className="text-xs text-accent">Gross margin</div>
                <div className="mt-1 font-mono text-base leading-tight text-foreground">{`${parseNumericInput(grossMarginPct).toFixed(0)}%`}</div>
              </div>
            </div>
          </CardContent>
        </Card>
        ...
        <div className="prose prose-sm sm:prose-base prose-zinc dark:prose-invert mt-6">
          ...
          <p>
            CAC payback is the time it takes for gross profit from a customer to
            recover the cost to acquire them. It connects acquisition efficiency
            to monetization quality.
          </p>
          ...
          <p>
            Payback (months) = CAC ÷ (ARPU × Gross margin%). Use monthly ARPU
            and margin for consistency.
          </p>
          <h3>Example</h3>
          <p>
            CAC {parseNumericInput(cac).toLocaleString()}, ARPU{" "}
            {parseNumericInput(arpu).toLocaleString()}, GM{" "}
            {parseNumericInput(grossMarginPct).toFixed(0)}% ⇒ Payback{" "}
          ...
          <h3>FAQ</h3>
          <p>
            <strong>What is a good CAC payback?</strong> Under 12 months for
            SMB; enterprise can tolerate longer with large deals.
          </p>
          <p>
            <strong>Should I use gross or net revenue?</strong> Use ARPU and
            gross margin to reflect delivery costs.
          </p>
          <p>
            <strong>How does churn affect payback?</strong> High churn reduces
            lifetime gross profit, effectively lengthening payback.
          </p>
        </div>
      </div>
    </div>

...
export default function BurnRateTool() {
  ...
  const netBurn = React.useMemo(() => {
    const inflow = parseNumericInput(cashInflow);
    const outflow = parseNumericInput(cashOutflow);
  ...
  return (
    <div>
      <div className="prose prose-sm sm:prose-base prose-zinc dark:prose-invert mt-6 tracking-wide">
        <h2>Burn Rate</h2>
        <p>
          Net burn is the monthly cash outflow minus inflow. Formula:{" "}
          <code>Net burn = Outflow − Inflow</code>. Use this calculator to
          understand monthly cash usage and its impact on runway. Provide
          monthly inflows and outflows to see net burn in dollars per month.
        </p>
      </div>
      ...
      <div className="mt-6 grid grid-cols-1 gap-4">
            ...
            <div className="grid grid-cols-2 md:grid-cols-4 gap-3 items-end">
              ...
              <div className="space-y-1">
                ...
                <Input
                  id="netburn"
                  type="text"
                  value={`${formatUsd(Math.max(netBurn, 0))} / month`}
                  readOnly
                />
              </div>
            </div>
          ...
          <CardContent>
            <div className="grid grid-cols-2 md:grid-cols-4 gap-4 items-stretch">
              <div className="rounded-md  border p-3 text-center flex flex-col items-center justify-center min-h-[72px]">
                ...
                <div className="mt-1 font-mono text-base leading-tight text-foreground">{`${formatUsd(Math.max(netBurn, 0))} / month`}</div>
              </div>
              <div className="rounded-md  border p-3 text-center flex flex-col items-center justify-center min-h-[72px]">
                <div className="text-xs text-accent">Inflow</div>
                <div className="mt-1 font-mono text-base leading-tight text-foreground">
                  {formatUsd(parseNumericInput(cashInflow))}
                </div>
              </div>
              <div className="rounded-md  border p-3 text-center flex flex-col items-center justify-center min-h-[72px]">
                <div className="text-xs text-accent">Outflow</div>
                <div className="mt-1 font-mono text-base leading-tight text-foreground">
                  {formatUsd(parseNumericInput(cashOutflow))}
                </div>
              </div>
              <div className="rounded-md  border p-3 text-center flex flex-col items-center justify-center min-h-[72px]">
                <div className="text-xs text-accent">Status</div>
                <div className="mt-1 font-mono text-base leading-tight text-foreground">
                  {netBurn <= 0 ? "Positive cash flow" : "Burning cash"}
                </div>
              </div>
            </div>
          </CardContent>
        ...
        <div className="prose prose-sm sm:prose-base prose-zinc dark:prose-invert mt-6">
          ...
          <p>
            Burn rate is the net cash outflow each month.{" "}
            <strong>Net burn</strong> = cash outflow − cash inflow. Some teams
            also track <em>gross</em> burn (outflow only), but net burn is more
            informative.
          </p>
          ...
          <p>
            Net burn = Monthly cash outflow − Monthly cash inflow. Report in
            absolute dollars per month.
          </p>
          <h3>Example</h3>
          <p>
            Outflow {formatUsd(parseNumericInput(cashOutflow))}/mo, inflow{" "}
            {formatUsd(parseNumericInput(cashInflow))}/mo ⇒ Net burn{" "}
            {`${formatUsd(Math.max(netBurn, 0))} / month`}.
          </p>
          ...
          <ul>
            ...
            <li>
              Increase cash collections (billing cadence, dunning, discounts for
              prepay).
            </li>
          </ul>
          <h3>FAQ</h3>
          <p>
            <strong>Which burn should I report—gross or net?</strong> Report
            net; it captures inflows and reflects actual cash usage.
          </p>
          <p>
            <strong>Is high burn always bad?</strong> Not if ROI is compelling
            and cash runway is sufficient; monitor carefully.
          </p>
          <p>
            <strong>How often should burn be tracked?</strong> Weekly for ops,
            monthly for reporting; alert on large spikes.
          </p>
        </div>
      </div>
    </div>

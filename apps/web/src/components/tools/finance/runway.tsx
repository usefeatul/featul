...
export default function RunwayTool() {
  ...
  const runwayMonths = React.useMemo(() => {
    const c = parseNumericInput(cash);
    const b = parseNumericInput(monthlyBurn);
    if (b <= 0) return 0;
    return c / b;
  }, [cash, monthlyBurn]);
  ...
  return (
    <div>
      <div className="prose prose-sm sm:prose-base prose-zinc dark:prose-invert mt-6 tracking-wide">
        <h2>Runway</h2>
        <p>
          Estimate months of runway given current cash and net monthly burn.
          Formula: <code>Runway = Cash ÷ Net burn</code>. Use this calculator to
          understand how long your cash reserves last at the current burn rate.
          Enter cash on hand and <em>net</em> monthly burn (outflow minus
          inflow) to see runway in months.
        </p>
      </div>
      ...
      <div className="mt-6 grid grid-cols-1 gap-4">
        <Card>
          <CardHeader className="space-y-1 tracking-wide">
            ...
            <CardDescription>
              Enter current cash and monthly net burn.
            </CardDescription>
          </CardHeader>
          <CardContent>
            <div className="grid grid-cols-2 md:grid-cols-4 gap-3 items-end">
              ...
              <div className="space-y-1">
                ...
                <Input
                  id="months"
                  type="text"
                  value={runwayMonths ? runwayMonths.toFixed(1) : "—"}
                  readOnly
                />
              </div>
            </div>
          ...
          <CardContent>
            <div className="grid grid-cols-2 md:grid-cols-4 gap-4 items-stretch">
              ...
              <div className="rounded-md  border p-3 text-center flex flex-col items-center justify-center min-h-[72px]">
                ...
                <div className="mt-1 font-mono text-base leading-tight text-foreground">
                  {formatUsd(parseNumericInput(cash))}
                </div>
              </div>
              <div className="rounded-md  border p-3 text-center flex flex-col items-center justify-center min-h-[72px]">
                <div className="text-xs text-accent">Net burn</div>
                <div className="mt-1 font-mono text-base leading-tight text-foreground">
                  {formatUsd(parseNumericInput(monthlyBurn))}/mo
                </div>
              </div>
            </div>
          </CardContent>
        ...
        <div className="prose prose-sm sm:prose-base prose-zinc dark:prose-invert mt-6">
          ...
          <p>
            Runway is the number of months your company can operate before cash
            runs out, given today’s
            <strong className="font-semibold"> net monthly burn</strong>. Use{" "}
            <em>net</em> burn (cash outflow minus cash inflow) rather than gross
            burn to avoid overstating risk.
          </p>
          ...
          <p>
            Runway = Current cash ÷ Net monthly burn. If net burn ≤ 0, you have
            non‑negative burn and effectively infinite runway while that
            condition holds.
          </p>
          <h3>Example</h3>
          <p>
            With {formatUsd(parseNumericInput(cash))} in cash and{" "}
            {formatUsd(parseNumericInput(monthlyBurn))}/mo burn, runway ≈{" "}
          ...
          <ul>
            ...
            <li>
              Improve gross margin (pricing, packaging, lower COGS) to reduce
              burn.
            </li>
          </ul>
          <h3>FAQ</h3>
          <p>
            <strong>Is runway based on net or gross burn?</strong> Use net burn
            (outflow − inflow). Gross burn ignores inflows and overstates risk.
          </p>
          <p>
            <strong>What’s a good runway?</strong> 12–18 months is common for
            SaaS to safely iterate and raise; longer if sales cycles are slow.
          </p>
          <p>
            <strong>How often should I recalculate?</strong> Monthly, or anytime
            burn or cash position changes materially.
          </p>
        </div>
      </div>
    </div>

...
export default function BreakEvenTool() {
  ...
  const marginPerUnit = React.useMemo(() => {
    return Math.max(
      parseNumericInput(price) - parseNumericInput(variableCost),
      0,
    );
  }, [price, variableCost]);
  ...
  const breakEvenUnits = React.useMemo(() => {
    ...
    if (m <= 0) return 0;
    return parseNumericInput(fixedCosts) / m;
  }, [fixedCosts, marginPerUnit]);

  const breakEvenRevenue = React.useMemo(() => {
    return breakEvenUnits * parseNumericInput(price);
  ...
  return (
    <div>
      <div className="prose prose-sm sm:prose-base prose-zinc dark:prose-invert mt-6 tracking-wide">
        ...
        <p>
          Units and revenue needed to break even. Formulas:{" "}
          <code>Margin per unit = Price − Variable cost</code>;{" "}
          <code>Break-even units = Fixed costs ÷ Margin per unit</code>. Use
          this calculator to find the sales volume or revenue required to cover
          fixed and variable costs. Enter price, variable cost per unit, and
          fixed costs.
        </p>
      </div>
      ...
      <div className="mt-6 grid grid-cols-1 gap-4">
        <Card>
          <CardHeader className="space-y-1 tracking-wide">
            ...
            <CardDescription>
              Enter price, variable cost and fixed costs.
            </CardDescription>
          </CardHeader>
          <CardContent>
        ...
        <div className="prose prose-sm sm:prose-base prose-zinc dark:prose-invert mt-6">
          ...
          <p>
            Break-even is the point where total revenue equals total costs, and
            profit is zero. It tells you how many units (or how much revenue)
            you need to cover fixed and variable costs.
          </p>
          ...
          <p>
            Margin per unit = Price − Variable cost. Break-even units = Fixed
            costs ÷ Margin per unit. Break-even revenue = Break-even units ×
            Price.
          </p>
          <h3>Example</h3>
          <p>
            Price {formatUsd(parseNumericInput(price))}, Variable cost{" "}
            {formatUsd(parseNumericInput(variableCost))}, Fixed costs{" "}
            {formatUsd(parseNumericInput(fixedCosts))} ⇒ Margin per unit{" "}
            {formatUsd(marginPerUnit)}, Break-even units{" "}
            {breakEvenUnits.toFixed(0)}, Break-even revenue{" "}
            {formatUsd(breakEvenRevenue)}.
          </p>
          ...
          <ul>
            ...
            <li>
              Lower variable costs through vendor negotiation or process
              improvements.
            </li>
            <li>
          ...
          <h3>FAQ</h3>
          <p>
            <strong>Is break-even the same as payback?</strong> No—break-even
            looks at units and revenue; payback focuses on recovering CAC via
            gross profit.
          </p>
          <p>
            <strong>Which costs count as fixed?</strong> Costs that don’t vary
            with units in the short term (e.g., salaries, rent).
          </p>
          <p>
            <strong>Can break-even change over time?</strong> Yes—pricing, cost
            structure, and volume mix shift margins and fixed costs.
          </p>
        </div>
      </div>
    </div>

  ...
  return j.router({
    ...
    checkSlug: privateProcedure
      ...
      .post(async ({ ctx, input, c }) => {
        ...
        const [resv] = await ctx.db
          .select({
            email: workspaceSlugReservation.email,
            status: workspaceSlugReservation.status,
            expiresAt: workspaceSlugReservation.expiresAt,
          })
        ...
        const active =
          ...
          resv.expiresAt &&
          resv.expiresAt.getTime() > now.getTime();
        const mine =
          active && String(resv?.email || "").toLowerCase() === meEmail;
    ...
    create: privateProcedure
      ...
      .post(async ({ ctx, input, c }) => {
        ...
        const [resv] = await ctx.db
          .select({
            id: workspaceSlugReservation.id,
            email: workspaceSlugReservation.email,
            status: workspaceSlugReservation.status,
            expiresAt: workspaceSlugReservation.expiresAt,
          })
        ...
        if (resv) {
          const expired =
            resv.expiresAt && resv.expiresAt.getTime() < Date.now();
          ...
          if (active && myEmail !== String(resv.email || "").toLowerCase()) {
            throw new HTTPException(403, {
              message: "Slug is reserved by another email",
            });
          }
        }
        ...
        try {
          ...
          try {
            ...
            const [r] = await ctx.db
              ...
              .where(
                and(
                  eq(workspaceSlugReservation.slug, slug),
                  eq(workspaceSlugReservation.email, myEmail),
                ),
              )
              .limit(1);
            if (r?.id) {
    ...
    updateCustomDomain: privateProcedure
      ...
      .post(async ({ ctx, input, c }) => {
        ...
        if (!(planKey === "starter" || planKey === "professional")) {
          throw new HTTPException(403, {
            message: "Custom domain available on Starter or Professional plans",
          });
        }
    ...
    createDomain: privateProcedure
      ...
      .post(async ({ ctx, input, c }) => {
        ...
        if (parts.length < 2)
          throw new HTTPException(400, { message: "Invalid domain host" });
        const cnameName = parts[0];

        const DEFAULT_CNAME_TARGET =
          process.env.CUSTOM_DOMAIN_CNAME_TARGET || "origin.featul.com";
        ...
        if (existing)
          throw new HTTPException(409, {
            message:
              "A domain is already configured. Delete it before adding a new one.",
          });
        await ctx.db
        ...
        return c.superjson({
          ...
          host,
          records: {
            cname: { name: cnameName, value: DEFAULT_CNAME_TARGET },
            txt: { name: txtName, value: token },
          },
        });
      }),
    ...
    verifyDomain: privateProcedure
      ...
      .post(async ({ ctx, input, c }) => {
        ...
        const [d] = await ctx.db
          ...
          .limit(1);
        if (!d)
          throw new HTTPException(404, { message: "No domain configured" });
        ...
        const status: "pending" | "verified" | "error" =
          cnameValid && txtValid ? "verified" : "pending";
        await ctx.db
          .update(workspaceDomain)
          .set({ status, lastVerifiedAt: new Date(), updatedAt: new Date() })
    ...
    updateName: privateProcedure
      ...
      .post(async ({ ctx, input, c }) => {
        ...
        if (!allowed) {
          ...
          allowed =
            Boolean(me) &&
            Boolean((me as any)?.permissions?.canManageWorkspace === true);
        }
        if (!allowed) throw new HTTPException(403, { message: "Forbidden" });
    ...
    delete: privateProcedure
      ...
      .post(async ({ ctx, input, c }) => {
        ...
        const expectedName = String(ws.name || "")
          .trim()
          .toLowerCase();
        const provided = String(input.confirmName || "")
          .trim()
          .toLowerCase();
    ...
    exportCsv: privateProcedure
      ...
      .get(async ({ ctx, input, c }) => {
        ...
        const rows = posts.map((p: PostRow) => {
          const attachments =
            p.metadata?.attachments?.map((a) => a.url).join("; ") || "";
          return [
            escapeCell(p.id),
            escapeCell(p.title),
        ...
        c.header(
          "Content-Disposition",
          `attachment; filename="${input.slug}-export.csv"`,
        );
        return c.body("\uFEFF" + csv);
      }),
    ...
    importFromCanny: privateProcedure
      ...
      .post(async ({ ctx, input, c }) => {
        ...
        if (!isDataImportsAllowed(String(ws.plan || "free"))) {
          throw new HTTPException(403, {
            message:
              "Canny import is available on Starter or Professional plans",
          });
        }
        ...
        return c.json(
          {
            ok: false,
            provider: "canny",
            message: "Canny import is coming soon",
          },
          501,
        );
      }),
    ...
    importFromNolt: privateProcedure
      ...
      .post(async ({ ctx, input, c }) => {
        ...
        if (!isDataImportsAllowed(String(ws.plan || "free"))) {
          throw new HTTPException(403, {
            message:
              "Nolt import is available on Starter or Professional plans",
          });
        }
        ...
        return c.json(
          {
            ok: false,
            provider: "nolt",
            message: "Nolt import is coming soon",
          },
          501,
        );
      }),
    ...
    importFromProductBoard: privateProcedure
      ...
      .post(async ({ ctx, input, c }) => {
        ...
        if (!isDataImportsAllowed(String(ws.plan || "free"))) {
          throw new HTTPException(403, {
            message:
              "ProductBoard import is available on Starter or Professional plans",
          });
        }
        ...
        return c.json(
          {
            ...
            message: "ProductBoard import is coming soon",
          },
          501,
        );
      }),

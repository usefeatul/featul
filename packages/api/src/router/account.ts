...
export function createAccountRouter() {
  return j.router({
    ...
    listDeviceAccounts: privateProcedure.get(async ({ ctx, c }) => {
      ...
      const currentImage =
        typeof ctx.session.user.image === "string"
      ...
      const accounts = buildDeviceAccounts({
        deviceSessions,
        currentUserId,
        currentToken,
        currentName,
    ...
    bootstrapDeviceSession: privateProcedure.post(async ({ ctx, c }) => {
      ...
      const listResult = await auth.api
        ...
        .catch(() => []);
      const deviceSessions = parseDeviceSessions(listResult);
      const existingAccounts = buildDeviceAccounts({
        deviceSessions,
        currentUserId,
        currentToken,
        currentName,
      ...
      if (existingAccounts.length >= MAX_DEVICE_ACCOUNTS) {
        throw new HTTPException(409, {
          message: `You can connect up to ${MAX_DEVICE_ACCOUNTS} accounts on this device`,
        });
      }
    ...
    revokeSession: privateProcedure
      ...
      .post(async ({ ctx, input, c }) => {
        ...
        const [target] = await ctx.db
          ...
          .from(session)
          .where(
            and(eq(session.id, input.sessionId), eq(session.userId, userId)),
          )
          .limit(1);
        ...
        const revokedCurrentSession =
          Boolean(currentToken) && String(target.token) === currentToken;
